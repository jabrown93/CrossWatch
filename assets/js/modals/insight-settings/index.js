...
const FEAT_META = {
  watchlist: ["Watchlist", "Show watchlist tiles."],
  ratings: ["Ratings", "Show ratings tiles."],
  history: ["History", "Show history tiles."],
  progress: ["Progress", "Show in-progress playback tiles."],
  playlists: ["Playlists", "Not used."],
};
...
const provLabel = (v) => ({ MDBLIST: "MDBList", TMDB: "TMDb" }[canonProv(v)] || `${canonProv(v).slice(0, 1)}${canonProv(v).slice(1).toLowerCase()}`);
...
const renderFeatures = (prefs) => FEATS.map((key) => {
  const [label, copy] = FEAT_META[key] || [key, ""];
